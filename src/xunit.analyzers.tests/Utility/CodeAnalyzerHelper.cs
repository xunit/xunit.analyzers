	...
	static CodeAnalyzerHelper()
	{
		...
		CurrentXunitV2 = defaultAssemblies.AddPackages(
			ImmutableArray.Create(
				...
				new PackageIdentity("xunit.assert", "2.5.0-pre.45"),
				new PackageIdentity("xunit.core", "2.5.0-pre.45")
			)
		);
		...
		CurrentXunitV3 = defaultAssemblies.AddPackages(
			ImmutableArray.Create(
				...
				new PackageIdentity("xunit.v3.assert", "0.1.1-pre.253"),
				new PackageIdentity("xunit.v3.common", "0.1.1-pre.253"),
				new PackageIdentity("xunit.v3.extensibility.core", "0.1.1-pre.253")
			)
		);
	}
}
