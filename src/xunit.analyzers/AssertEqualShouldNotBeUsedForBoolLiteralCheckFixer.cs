...
namespace Xunit.Analyzers
{
    ...
    public class AssertEqualShouldNotBeUsedForBoolLiteralCheckFixer : CodeFixProvider
    {
        ...
        public sealed override ImmutableArray<string> FixableDiagnosticIds { get; } = ImmutableArray.Create(Constants.Descriptors.X2004_AssertEqualShouldNotUsedForBoolLiteralCheck.Id);
        ...
        static string GetReplacementMethodName(string methodName, string literalValue)
        {
            ...
            if (AssertEqualShouldNotBeUsedForBoolLiteralCheck.EqualMethods.Contains(methodName))
                replacement = literalValue == bool.TrueString ? "True" : "False";
            else if (AssertEqualShouldNotBeUsedForBoolLiteralCheck.NotEqualMethods.Contains(methodName))
                replacement = literalValue == bool.TrueString ? "False" : "True";
            return replacement;
        }
