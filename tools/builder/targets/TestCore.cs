...
public static class TestCore
{
	public static Task OnExecute(BuildContext context)
	{
		...
		return context.Exec("dotnet", $"test src/xunit.analyzers.tests --framework netcoreapp2.1 --configuration {context.ConfigurationText} --no-build --logger trx;LogFileName=netcore.trx --results-directory \"{resultPath}\" --verbosity {context.Verbosity}");
	}
}
